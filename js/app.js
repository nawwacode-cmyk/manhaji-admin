...
window.App = (function () {
  ...
  const NAV = [
    ...
    // صفحة المزوّد الوحيدة — لا يملك سواها، فتظهر له وحدها ويُخفى كل ما عداها.
    { sec: 'البيع' },
    { id: 'myCodes',   label: 'أكوادي',        ico: () => icon.key(19) },
    { sec: 'الاشتراكات' },
    { id: 'seasons',   label: 'المواسم',       ico: () => icon.exam(19) },
    { id: 'codes',     label: 'أكواد التفعيل', ico: () => icon.key(19) },
    { id: 'providers', label: 'المزوّدون',     ico: () => icon.users(19) },
    { sec: 'الإدارة' },
    { id: 'students',  label: 'الطلاب',        ico: () => icon.users(19) },
    { id: 'staff',     label: 'الطاقم',        ico: () => icon.user(19) },
    { id: 'errors',    label: 'أعطال التطبيق', ico: () => icon.warn(19) },
  ...
  const TITLES = {
    ...
    codes:     ['أكواد التفعيل', 'كودٌ واحد لكل طالب — باسمه ورقمه وموسمه'],
    seasons:   ['المواسم الدراسية', 'الاشتراك ينتهي بنهاية موسمه لا بعد مدّة ثابتة'],
    myCodes:   ['أكوادي', 'بيع الأكواد للطلاب من رصيدك، ومتابعة ما بعته'],
    providers: ['المزوّدون', 'من يبيع الأكواد نيابةً عنّا، ورصيد كلٍّ منهم'],
    students:  ['الطلاب', 'الاشتراكات والأجهزة وأدوات الدعم'],
    staff:     ['الطاقم', 'حسابات الأساتذة والمديرين وصلاحياتهم'],
    errors:    ['أعطال التطبيق', 'ما ينكسر عند الطلاب — مجمَّعًا بالعطل لا بالوقت'],
  ...
  function render() {
    ...
    // حارس الصلاحيات: من يصل لصفحة لا يملكها (عبر رابط قديم مثلًا) يُعاد
    // إلى **أوّل صفحة يملكها هو** لا إلى لوحة المعلومات دائمًا. المزوّد لا
    // يملك dashboard إطلاقًا، فتثبيت الوجهة عليها كان يتركه أمام صفحة فارغة.
    if (!Store.can(current)) {
      current = Store.can('dashboard') ? 'dashboard'
        : (NAV.find((it) => it.id && Store.can(it.id))?.id || 'dashboard');
    }
