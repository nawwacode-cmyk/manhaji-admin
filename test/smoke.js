...
(async () => {
     ...
     ========================================================================= */
  {
    ...
    /* الحارس كان يشترط `disabled`، وقد أُزيل عمدًا: زرٌّ معطَّل لا يقول لماذا،
       فيظنّ المحرِّر أن الحفظ معطوب بينما السبب أنه لم يرفع ملفًّا. المطلوب
       أن يُمنع الوضعُ **مع تفسير**، لا أن يُمنع النقر. */
    ok('ووضعٌ يطلب ملفّات يُرفض بلا ملفّ مع تفسير',
       /if \(next !== 'text' && !docs\.length\)[\s\S]{0,60}ارفع ملفًّا أوّلًا/.test(content));

    /* --- ملفّ المادة: نوطة وملحق لا يتبعان درسًا -----------------------------
       القاعدة تفرض مالكًا **واحدًا بالضبط** (`lesson_docs_one_owner`)، فإرسال
       الاثنين معًا يعود بـ23514 ورسالةٍ لا يفهمها أحد. الحسم في اللوحة: درسٌ
       إن وُجد، وإلّا المادة. */
    ok('الرفع يقبل مالكًا مادةً كما يقبل درسًا',
       /subjectId = null/.test(up) && /subject_id: lessonId \? null : \(subjectId \|\| null\)/.test(commit),
       commit.slice(0, 90));
    // واسم الملفّ لا يُملأ باسم المادة: «العلوم» ليس اسمًا لملفٍّ داخلها
    ok('ولا يُملأ اسم الملفّ باسم المادة', /if \(lessonTitle && !subjectId\)/.test(up));

    const subjSrc = fs.readFileSync(dir + 'pages/subjects.js', 'utf8');
    ok('وللمادة مديرُ ملفّات في اللوحة',
       /onclick: \(\) => files\(sub\)/.test(subjSrc) && /subjectId: sub\.id/.test(subjSrc));
    // نفس الحذف الحقيقي: الصفّ والكائن في التخزين معًا لا حذفٌ ناعم
    ok('وحذفه حقيقي ومُعلَن',
       /action: 'remove'/.test(subjSrc) && /لا يمكن التراجع/.test(subjSrc));

    /* --- وصف المادة ---------------------------------------------------------
       حقلٌ لا يصل القاعدة أسوأ من غيابه: المحرِّر يكتب ويحفظ ويرى «حُفظت
       التعديلات» ثم يفتح فيجده فارغًا. الطريق كاملًا: الحقل ← الحمولة ←
       صفّ القاعدة ← القراءة العائدة. */
    ok('وصف المادة حقلٌ في المحرّر', /const desc\s*=\s*C\.textarea\(/.test(subjSrc));
    ok('ويصل حمولة الحفظ', /description: desc\.value\.trim\(\) \|\| null/.test(subjSrc));
    ok('ويُكتب في صفّ القاعدة', /description: row\.description \|\| null/.test(storeSrc));
    ok('ويُقرأ عائدًا إلى المحرّر', /description: s\.description \|\| ''/.test(storeSrc));
  }

  /* =========================================================================
     إطار قصّ صورة الأستاذ يطابق غلاف شاشة المادة

     القاعدة الخامسة في CLAUDE.md: «المعاينة تطابق ما يراه الطالب». معاينةٌ
     بنسبةٍ أخرى تجعل المحرِّر يضبط موضع الوجه على إطارٍ لا وجود له، فيوافق
     على قصٍّ لن يراه — وهو خطأٌ صامت لا يظهر إلّا في هاتف طالب.

     والفحص **يعبر المستودعين**: النسبة مكتوبة هنا وفي `app.css` هناك،
     وانحرافهما هو بالضبط ما تحرسه القاعدة. إن غاب مستودع التطبيق يُتخطّى
     الفحص بوضوح بدل أن يمرّ صامتًا.
     ========================================================================= */
  {
    const tSrc = fs.readFileSync(dir + 'pages/teachers.js', 'utf8');
    const ratio = (tSrc.match(/ratio: '([^']+)'/) || [])[1];
    ok('إطار القصّ مربّع', ratio === '1 / 1', ratio || 'لا نسبة');
    // والمعاينة فوقه بنفس النسبة، وإلّا اختلف ما يراه المحرِّر عن ما يضبطه
    ok('والمعاينة بنفس النسبة', /aspect-ratio:1\/1/.test(tSrc));

    const appCss = [
      process.env.MANHAJI_APP && process.env.MANHAJI_APP + '/css/app.css',
      (process.env.USERPROFILE || process.env.HOME || '') + '/Desktop/manhaji/css/app.css',
      dir + '../../manhaji/css/app.css',
    ].filter(Boolean).find((p) => fs.existsSync(p));

    if (!appCss) {
      console.log('⏭  تُخطّي مطابقة نسبة الغلاف: مستودع التطبيق غير موجود (اضبط MANHAJI_APP)');
    } else {
      const band = (fs.readFileSync(appCss, 'utf8')
        .match(/\.chero__band \{[^}]*height:\s*([^;]+)/) || [])[1] || '';
      /* الغلاف مربّع حين يُقاس بـ100vw: ارتفاعه = عرض الشاشة. أي قيمةٍ أخرى
         تعني نسبةً غير 1/1 فيلزم تحريك إطار القصّ معها. */
      ok('ونسبة الغلاف في التطبيق ما زالت مربّعة', /\b100vw\b/.test(band), band.trim());
    }
  }
  /* =========================================================================
